...
namespace YG.Insides
{
  public static partial class Langs
  {
        ...
        public const string t_autoDefineSymbols = "Для корректной работы плагина данная опция должна быть активна, не выключайте её без необходимости!\n\nПлагин автоматически корректирует список Define Symbols для контроля над компиляцией кода, чтобы настраивать зависимости.\n\nВам может понадобиться выключить данный параметр для углубленной мультиплатформенной разработки, или при возникновении ошибок компиляции, связанных с не правильным импортом или удалением модулей плагина.";
        public const string t_syncInitSDK = "Для платформы Яндекс Игры опцию Sync Init SDK можно не включать. Но для некоторых других веб-платформ может потребоваться активировать данную опцию. Смотрите документацию для каждой платформы. \n\n • False — сначала пройдёт полная инициализация SDK платформы, затем игра начнёт загружаться. Это даёт наибольшее удобство и гарантию, что при старте все функции SDK доступны для использования. Но загрузка при таком методе может быть немного дольше. \n\n • True — инициализация платформы будет происходить одновременно с загрузкой игры или после загрузки игры, это зависит от платформы. При таком сценарии проект должен быть готов к тому, что при старте игры SDK платформы и, соответственно, API плагина ещё не загрузились и не готовы для использования. Необходимо подождать, пока SDK инициализируется, затем пользоваться API плагина. Хорошим решением будет создать пустую нулевую сцену и запускать рабочую сцену после всех инициализаций. Или грузите рабочую сцену, но после инициализации перезагружайте её, но тогда хорошо проверьте, что такой способ не вызывает ошибок! Для загрузки сцены после инициализации есть готовое решение, воспользуйтесь параметрами в выпадающем списке ниже.";
        ...
        public const string t_initDelaySimulation = "Симуляция для Unity Editor при которой SDK инициализируется через одну секунду после запуска игры. Примерно повторяет поведение не мгновенной инициализации SDK платформы (надо подождать пока данные загрузятся, потом использовать функционал плагина и соответственно функционал платформы.";
    ...
    public const string t_autoDefineSymbols = "For the plug-in to work correctly, this option must be active, do not turn it off unnecessarily! \n\nThe plugin automatically adjusts the Define Symbols list to control code compilation in order to configure dependencies. \n\nYou may need to disable this option for in-depth multiplatform development, or if compilation errors occur due to incorrect import or removal of plug-in modules.";
    public const string t_syncInitSDK = "You don't need to enable the Sync Init SDK option for the Yandex Games platform. However, for some other web platforms, you may need to activate this option. See the documentation for each platform. \n\n • False — the platform SDK will be fully initialized first, then the game will start loading. This provides the greatest convenience and guarantees that all SDK functions are available for use at startup. However, it may take a little longer to load using this method. \n\n • True — initialization of the platform will occur simultaneously with the download of the game or after the download of the game, it depends on the platform. In this scenario, the project should be prepared for the fact that when the game starts, the SDK of the platform and, accordingly, the API of the plugin have not yet loaded and are not ready for use. You need to wait until the SDK is initialized, then use the plugin API. A good solution would be to create an empty null stage and run the production stage after all initializations. Or load the production stage, but reboot it after initialization, but then make sure that this method does not cause errors! To load the scene after initialization, there is a ready-made solution, use the options in the drop-down list below.";
    ...
    public const string t_initDelaySimulation = "A simulation for Unity Editor in which the SDK is initialized one second after the game is launched. Approximately repeats the behavior of non-instant initialization of the SDK platform (you need to wait until the data is loaded, then use the functionality of the plugin and, accordingly, the functionality of the platform.";
